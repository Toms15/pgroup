<?php
/**
 * Plugin Name:  Redirection For Contact Form 7 Extension - Custom Registration forms
 * Plugin URI:   https://redirection-for-contact-form7.com/
 * Description:  Create registration form instead Of WordPress default Registration screen.
 * Version:      2.3
 ...
 * @author Query Solutions
 *
 * Class WPCF7R_Action_register file.
 *
 * @package cf7r
 */
...
add_action( 'plugins_loaded', 'wpcf7r_register_loader', 30 );

define( 'WPCF7R_REGISTER_URL', plugin_dir_url( __FILE__ ) );
 ...
 * @return void
 */
function wpcf7r_register_loader() {
	...
	if ( ! class_exists( 'WPCF7R_Action' ) ) {
		...
		add_action(
			...
			<div class="notice notice-error">
				<p><a href="https://wordpress.org/plugins/wpcf7-redirect/" target="_blank"><?php _e( 'Redirection For Contact Form 7 Extension - Custom Registration forms - Please Download And Activate Redirection For Contact Form 7.' ); ?></a></p>
			</div>
				<?php
			},
			10
	...
	class WPCF7R_Action_register extends WPCF7R_Action {
		...
		public function get_action_fields() {
			...
			$roles = array_merge( array( '' => __( 'Select', 'wpcf7-redirect' ) ), $roles );
			...
			$fields = array_merge(
				array(
					...
					array(
						...
						'class'       => $this->get( 'send_notification' ) ? 'qs-col qs-col-12 email_notification' : 'qs-col qs-col-12 field-hidden email_notification',
					),
					array(
						'name'        => 'email_body',
						'type'        => 'editor',
						...
						'class'       => $this->get( 'send_notification' ) ? 'qs-col qs-col-12 email_notification' : 'qs-col qs-col-12 field-hidden email_notification',
						'footer'      => '<div>You can combine cf7 tags([tag1] [tag2]) use [auto-gen-password] to add auto generated password</div><div>' . $this->get_formatted_mail_tags() . '</div>',
					),
				),
				$parent_fields
			);
		...
		public function process_validation( $submission ) {
			...
			// Validate username.
			if ( isset( $mapped_user_data['user_login'] ) && $mapped_user_data['user_login']['value'] ) {
				if ( username_exists( $mapped_user_data['user_login']['value'] ) || email_exists( $mapped_user_data['user_login']['value'] ) ) {
					$api_result['invalid_tags'][] = $this->get_validation_error( $mapped_user_data['user_login']['tag'], __( 'Username/Email Already exists', 'wpcf7-redirect' ) );
				}
			}

			if ( ! isset( $mapped_user_data['user_login']['value'] ) && ! isset( $mapped_user_data['user_email']['value'] ) ) {
				$api_result['invalid_tags'][] = $this->get_validation_error( $mapped_user_data['user_login']['tag'], __( 'Username/Email Is required', 'wpcf7-redirect' ) );
		...
		public function welcome_email( $user_id ) {
			...
			$body = str_replace( '[auto-gen-password]', $password, $body );
		...
		private function get_password() {
			return isset( self::$data['mapped_data']['user_pass']['value'] ) ? self::$data['mapped_data']['user_pass']['value'] : '';
		}
		/**
